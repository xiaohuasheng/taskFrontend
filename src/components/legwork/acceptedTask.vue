...
<!--          <h2>Thinking</h2>-->
      ...
      <div class="memos">
        <task-item v-for="item in posts" v-bind:key="item.id" v-bind:think="item" v-bind:scene="'accepted'"></task-item>
...
export default {
  ...
  methods: {
    loadPage(page) {
      ...
      this.isLoading = true
      this.axios.get(process.env.BACKEND_HOST + '/api/legwork/task?status=accepted&id=' + thinkID + '&page=' + page).then(response => {
